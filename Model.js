...
// Multi-link support
//
// The stock panel collapses connectivity to a single link: it picks the first
// connected device and renders one icon. That is wrong on any machine with more
// than one link up (two Wi-Fi radios, or Wi-Fi plus Ethernet) -- only the
// default-route link is visible and the others are silently dropped.
//
// These helpers take a flat list of link samples and produce the bar's glyph
// run and tooltip. They stay pure ES5 so they are Node-testable: QML enums
...
function linkLabel(link) {
  var value = link || {}
  if (value.kind === "wifi") return value.ssid || "Wi-Fi"
  if (value.kind === "ethernet") return "Ethernet"
  return value.iface || "Unknown"
}
...
// Primary (default route) first so the link actually carrying traffic leads the
// bar, then wired, then strongest signal. Interface name breaks ties to keep
// ordering stable across polls -- otherwise two radios at equal strength would
// swap places and make the bar jitter.
function sortLinks(links) {
  var rows = Array.isArray(links) ? links.slice() : []
...
function collectLinks(samples) {
  var rows = Array.isArray(samples) ? samples : []
  var links = []

  for (var i = 0; i < rows.length; i++) {
    if (!isPresentableLink(rows[i])) continue
    links.push(rows[i])
  }
...
function linkDetail(link) {
  var value = link || {}
  if (value.kind === "wifi") {
    var signal = parseInt(value.signal, 10)
    return isFinite(signal) && signal >= 0 ? signal + "%" : ""
  }
  if (value.kind === "ethernet") return formatHeaderSpeed(value.speed || "")
  return ""
}

// "HomeNet (72%) · wlp3s0 · 192.0.2.24" per line, so hovering the bar
// names every link instead of just the default route.
function linkTooltipLine(link) {
  var value = link || {}
  var parts = [linkLabel(value)]
  var detail = linkDetail(value)
...
function barTooltip(links) {
  var rows = Array.isArray(links) ? links : []
  if (rows.length === 0) return "Disconnected"

  var lines = []
  for (var i = 0; i < rows.length; i++) lines.push(linkTooltipLine(rows[i]))
...
function linkPillLabel(link) {
  ...
  if (value.kind === "ethernet") return iface || "Ethernet"
  return iface
}

function linkPillTooltip(link) {
  var value = link || {}
  var line = linkTooltipLine(value)
  return value.primary ? line + " · default route" : line
}

// Which pill starts selected, and what to fall back to when the selected card
// goes away. Prefers the default-route link so the panel opens on the card the
// machine is actually using.
function defaultLinkIface(links) {
  var rows = Array.isArray(links) ? links : []
  for (var i = 0; i < rows.length; i++) {
    if (rows[i] && rows[i].primary) return rows[i].iface || ""
  }
  return rows.length > 0 ? (rows[0].iface || "") : ""
}

function hasLinkIface(links, iface) {
  var rows = Array.isArray(links) ? links : []
  if (!iface) return false
  for (var i = 0; i < rows.length; i++) {
    if (rows[i] && rows[i].iface === iface) return true
  }
  return false
}

function linkForIface(links, iface) {
  var rows = Array.isArray(links) ? links : []
  for (var i = 0; i < rows.length; i++) {
    if (rows[i] && rows[i].iface === iface) return rows[i]
  }
  return null
}

function linkIndexForIface(links, iface) {
  var rows = Array.isArray(links) ? links : []
  for (var i = 0; i < rows.length; i++) {
    if (rows[i] && rows[i].iface === iface) return i
  }
  return -1
}

// Interface names reach a shell argv through the bundled helper, so anything
// outside the kernel's own character set is refused rather than quoted.
function isSafeIfaceName(name) {
  return typeof name === "string" && /^[A-Za-z0-9._-]+$/.test(name)
}

// Absolute path to this plugin's directory, from Qt.resolvedUrl(".").
//
// Rejects any non-file URL scheme and any traversal segment, literal or
// percent-encoded. A URL that fails those checks yields "", which disables the
// helper rather than resolving an executable somewhere else on disk.
function pluginDirFromUrl(url) {
  ...
  if (decoded === "" || decoded.charAt(0) !== "/") return ""
  if (decoded.indexOf("\0") !== -1) return ""
...
// Absolute path to a bundled helper, or "" when the plugin dir is untrustworthy.
function helperPath(pluginDir, name) {
...
if (typeof module !== "undefined") {
  module.exports = {
    ...
    hasLinkIface: hasLinkIface,
    linkForIface: linkForIface,
    linkIndexForIface: linkIndexForIface,
    isSafeIfaceName: isSafeIfaceName,
