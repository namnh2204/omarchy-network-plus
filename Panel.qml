...
Panel {
  ...
  // Bundled helper lives in this plugin's own bin/, because `omarchy plugin add`
  // copies the folder as-is and nothing is installed onto PATH. Resolving from
  ...
  // with no safe selection it is called bare and reports the default route,
  // which matches stock behavior.
  function startDetails() {
    if (detailsProc.running) return
    if (linkStatusHelper === "") {
      // No trustworthy helper path: fall back to the stock command rather than
      // leaving the panel with no stats at all.
      detailsProc.command = ["omarchy-network-status", "--verbose"]
      detailsProc.running = true
      return
    }
    var iface = root.effectiveIface
    detailsProc.command = Model.isSafeIfaceName(iface)
      ? [linkStatusHelper, iface]
      : [linkStatusHelper]
    detailsProc.running = true
  }
  ...
  function updateDetails(raw) {
    ...
    // Tracked separately from info.iface: info describes the card being shown,
    // while this is the card actually carrying the default route, which is what
    // marks a pill primary.
    if (next.default_iface !== undefined) defaultIface = next.default_iface
  ...
  // Thin wrappers so the multi-link delegates stay declarative, matching how
  // wifiIconFor/wifiSectionTitle already front Model.js for the scan rows.
  function linkIcon(link) {
    return Model.linkIcon(link)
  }

  function linkPillLabel(link) {
    return Model.linkPillLabel(link)
  }

  function linkPillTooltip(link) {
    return Model.linkPillTooltip(link)
  }
  ...
  Process {
    id: detailsProc
    // Command is assigned per run by startDetails(), which targets the selected
    // interface instead of always reporting the default route.
  ...
  BarIconButton {
    ...
    anchors.fill: parent
    bar: root.bar
    // Single glyph, as stock: the bar stays one slot wide. Multi-link state is
    // surfaced in the tooltip and in the panel's card selector instead.
    text: root.icon
    // Names every active link, so two connected radios are visible on hover
    // even though the bar itself shows one icon.
    ...
    PanelKeyCatcher {
      ...
      onMoveRequested: function(dx, dy) {
        ...
        if (dy !== 0) {
          ...
          // links and band sections dropping out of the chain entirely when
          // they aren't on screen.
          if (root.focusSection === "header") {
            if (dy > 0) {
              if (root.canSelectLink) {
  ...
  component LinkPill: Button {
    ...
    onClicked: root.selectLink(linkPill.link.iface)
